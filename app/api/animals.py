...
def update_animal(
    ...
    previous = db.get_animal(animal_id)
    if not previous:
        raise HTTPException(status_code=404, detail="Animal não encontrado")

    updated = db.update_animal(
        animal_id, body.name, body.description, body.breed, body.weight, body.status
    )
    if not updated:
        raise HTTPException(status_code=404, detail="Animal não encontrado")

    # Lançamento financeiro automático ao marcar como vendido ou abatido
