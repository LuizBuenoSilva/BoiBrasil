...
def list_vaccines(
    ...
    return db.list_vaccines(animal_id)
...
def add_vaccine(body: VaccineCreate, current_user: dict = Depends(get_current_user)):
    ...
    vaccines = db.list_vaccines(body.animal_id)
...
def delete_vaccine(vaccine_id: int, current_user: dict = Depends(get_current_user)):
    if not db.delete_vaccine(vaccine_id):
...
def upcoming_vaccines(
    ...
    """Vacinas com vencimento nos próximos N dias."""
    rows = db.list_upcoming_vaccines(days)
    # Complementa com campos extras ausentes na query simplificada
    return [
